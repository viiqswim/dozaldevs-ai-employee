...
| Engineering worker libraries    | `src/workers/lib/` (except `postgrest-client.ts` and `session-manager.ts`) | 30 utilities exclusively supporting `orchestrate.mts` (wave executor, PR manager, session manager, etc.). On hold — do not modify. `postgrest-client.ts` is shared with `opencode-harness.mts` and is active. `session-manager.ts` is also active — imported by `opencode-harness.mts` to manage OpenCode sessions. **`postgrest-client.ts` uses raw `process.env` with null-checks intentionally** — worker startup guarantees differ from gateway startup; do not "fix" with `requireEnv()`. |
