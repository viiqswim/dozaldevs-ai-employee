...
- **Declared inputs are `{{key}}` placeholders (generator/wizard path)**: When the archetype generator or wizard creates an employee with declared inputs (e.g. `input_schema` item with `key: "target_date"`), the generated `execution_steps` reference the value as `{{target_date}}` (matching the snake*case key). The platform resolves every `{{key}}` to the actual runtime value via `substituteTemplateVars` on the compiled AGENTS.md, fed by `INPUT*<KEY>`env vars, **before the model runs**. Generated steps must NOT contain`printenv INPUT*TARGET_DATE`, `node -e "...getUTCDay..."`, or any env-var reads — that is leaked plumbing. This applies to any input key: `{{report_date}}`, `{{customer_name}}`, etc. **Legacy/manual-archetype path**: hand-authored archetypes that read `INPUT*<KEY>`directly via`printenv`are still supported. When the compiler detects a`printenv INPUT\_`reference in`execution_steps`, it injects `DATE_PARAMETERIZATION_RULES`into the compiled AGENTS.md so the worker knows how to handle it. Do not add this plumbing to generator-produced steps — the`{{key}}` substitution happens at the platform layer before the model ever sees the prompt.
