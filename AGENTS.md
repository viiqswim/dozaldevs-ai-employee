...
**AI Assistant tab**: `/dashboard/employees/:id?tab=assistant` — non-technical users can request conversational changes to an employee's behavior (prose, tools, schedule, approval gate, required inputs). Changes are previewed as diffs, applied on Approve via `PATCH /admin/tenants/:tenantId/archetypes/:id`, and recorded in `archetype_edit_history`. Distinct from `employee_rules` (learned rules from Slack feedback) — this feature edits archetype prose directly.
...
- **`archetype_edit_history` table**: `id, archetype_id, tenant_id, request_text (Text), before_json (Json), after_json (Json), changed_fields (Json — string[]), kind (String — 'edit' | 'revert'), actor_user_id (String? — nullable for SERVICE_TOKEN), created_at (default now), deleted_at (DateTime?)`. Append-only audit trail for the AI Assistant conversational editing feature. Indexed on `[archetype_id, created_at]` and `[tenant_id]`. Soft-delete only — never hard-delete rows. `actor_user_id` is null when the change was made via SERVICE_TOKEN.
