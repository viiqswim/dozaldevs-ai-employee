...
│   └── lib/          # `agents-md-compiler.mts` (template compiler), `postgrest-client.ts` (shared DB client), `postgrest-types.ts` (typed PostgREST row interfaces — snake_case, use for all PostgREST reads/writes), `harness-helpers.mts` (extracted harness utilities: container naming, log helpers), `execution-phase.mts` (execution phase logic + `isToolAllowed()` capability enforcement), `delivery-phase.mts` (delivery phase logic extracted from harness), `output-schema.mts` (Zod schema for `StandardOutput` — includes optional `version` field), `opencode-server.ts` (spawns + health-checks the local OpenCode HTTP server, manages TCP reconnect and graceful shutdown), `session-manager.ts` (drives an OpenCode session to completion), `prompt-assembler.mts` (assembles the task prompt: dynamic date line + instructions + mandatory submit-output suffix + task ID), `template-vars.ts` (`buildTemplateVars()` — lowercases env vars and expands `INPUT_*` for prompt templating), `trigger-payload.mts` (`extractTriggerPrompt()` / `injectAssignmentSection()` — pulls the prompt from the trigger payload into the instructions), `model-provider.mts` (`resolveModelProvider()` — strips `openrouter/` prefix and picks Go vs OpenRouter), `env-manifest-builder.mts` (`buildEnvManifestFromVars()` — formats the env-var manifest for AGENTS.md injection, shared with brain-preview), `heartbeat.ts` (60-second execution heartbeat timer with stage updates), `approval-card-poster.mts` (builds + posts the Slack approval card from `StandardOutput`), `slack-notifier.mts` (`updateSlackNotificationToFailed()` — non-fatal Slack failure-state update), `output-contract.mts` (reads + validates the `/tmp` output-contract files after a session), `failure-codes.ts` (`FAILURE_CODES` — canonical failure-code strings for `executions.failure_code`), `resource-caps.ts` (`RESOURCE_CAPS` — battle-tested container resource limits: Turbo/Vitest concurrency, bash timeout, Node heap)
