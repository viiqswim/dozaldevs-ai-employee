#!/usr/bin/env tsx
/**
 * seed-nonowner-user.ts — Bootstrap a USER-role test account in Supabase Auth and the app DB.
 *
 * Used for E2E negative testing — verifies that non-owner users cannot access owner-only endpoints.
 *
 * Idempotent: produces exactly 1 USER-role account with MEMBER memberships in both tenants.
 *
 * Required env: BOOTSTRAP_NONOWNER_EMAIL, SUPABASE_URL, SUPABASE_SECRET_KEY, DATABASE_URL
 * Optional env: BOOTSTRAP_NONOWNER_PASSWORD (defaults to a random strong password, printed once)
 *
 * Usage:
 *   BOOTSTRAP_NONOWNER_EMAIL=testuser@example.com BOOTSTRAP_NONOWNER_PASSWORD=Test1234! tsx scripts/seed-nonowner-user.ts
 */
...
export interface SeedNonOwnerResult {
  email: string;
  supabaseId: string;
  appUserId: string;
...
export async function seedNonOwnerUser(): Promise<SeedNonOwnerResult> {
  const email = process.env.BOOTSTRAP_NONOWNER_EMAIL;
  if (!email) {
    throw new Error('BOOTSTRAP_NONOWNER_EMAIL env var is required');
  }
  ...
  const password = process.env.BOOTSTRAP_NONOWNER_PASSWORD ?? randomBytes(16).toString('hex');
  ...
  try {
    // Create app user with global role USER (not PLATFORM_OWNER)
    ...
    // Grant MEMBER (not OWNER/ADMIN) memberships in both tenants
    ...
    for (const tenantId of tenantIds) {
      await prisma.tenantMembership.upsert({
        ...
        create: { tenant_id: tenantId, user_id: user.id, role: 'MEMBER' },
...
if (isMain) {
  ...
  const generatedPassword = !process.env.BOOTSTRAP_NONOWNER_PASSWORD;
  if (generatedPassword) {
    const pw = randomBytes(16).toString('hex');
    process.env.BOOTSTRAP_NONOWNER_PASSWORD = pw;
    console.log('\n⚠  No BOOTSTRAP_NONOWNER_PASSWORD set — generated:');
    console.log(`   ${pw}`);
  ...
  seedNonOwnerUser()
    .then((result) => {
      console.log('\n✓ Non-owner test user created:');
      console.log(`  email=${result.email}`);
      console.log(`  supabase_id=${result.supabaseId}`);
      console.log(`  app_user_id=${result.appUserId}`);
      console.log(`  global_role=USER`);
      console.log(`  memberships=[DozalDevs(MEMBER), VLRE(MEMBER)]`);
    })
    .catch((err: unknown) => {
