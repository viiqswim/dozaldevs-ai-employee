...
3. **UNASSIGNED vs. backup-fallback — two distinct cases**: State: "For every work item whose key IS in the covered set: assign to the primary if available and within capacity; if the primary is unavailable (day off) or over capacity, assign to the roster-defined backup for that key — do NOT mark UNASSIGNED. Mark UNASSIGNED ONLY when the key is absent from the covered set entirely (no primary AND no backup in the roster). Do NOT infer coverage from geographic proximity, zone groupings in non-roster directories, section headers, or any other non-authoritative source."
