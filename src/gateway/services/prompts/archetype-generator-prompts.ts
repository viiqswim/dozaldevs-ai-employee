...
CRITICAL — emit these steps DEFENSIVELY: at generation time you do NOT know what the reference data contains (you have not read it). For any roster/team-based employee that reads reference data, you MUST emit the capacity-and-backup-application step UNCONDITIONALLY — phrase it so it self-handles the empty case: "IF the reference data defines a per-person capacity limit, apply the over-capacity → backup logic below; if no capacity limit is found for a person, treat them as uncapped." NEVER omit the capacity/backup step just because the user's description did not mention capacity limits — the limits live in the reference data, which the description rarely restates. Omitting the step because the description is silent is the most common failure and is FORBIDDEN.

Specifically:
- **Capacity limits in reference data**: For any roster employee, the execution_steps MUST include a dedicated numbered step (emitted unconditionally) that: (a) reads any per-person working-hours/capacity limits from the reference source and declares each team member's limit aloud (or "uncapped" if none is listed), (b) calculates total work time per team member by summing all assigned properties, (c) if total exceeds that person's capacity limit, ACTUALLY ASSIGNS the overflow to the roster-defined backup person — do NOT merely note the overflow, flag it, or recommend backup; the step MUST say "MOVE [overflow properties] from [primary] to [backup person] and list them under [backup person]" using property-address grouping (smallest-group-first, never split a single address across people). A step that only notes/flags the overflow without re-listing the moved properties under the backup is FORBIDDEN — the reassignment MUST be made in the step itself, and the final compiled schedule MUST show the overflow properties under the backup, not under the over-capacity primary.
...
2. **Extract a lookup table explicitly AND declare the covered-key set aloud** — after reading, parse the content to build a structured lookup table. The step MUST say exactly what to extract and how. The content may be prose, bullets, or a table — reason over whatever structure is present. Then immediately declare the complete, finite set of covered keys (zones/areas/codes) that the roster actually lists. The generated step MUST include this exact sentence (adapted to the actual key type): "This set is now CLOSED — a property is covered ONLY if its exact [key type] appears in this set. Do NOT group nearby [keys]. Do NOT infer coverage from [other sources]." Example:
   > "Read the full content of Notion page [exact-id]. Parse it to build a ZIP-code → cleaner mapping by reading each section and extracting area codes and assigned names. Declare the full extracted table aloud. Then declare the complete covered-key set: 'Covered ZIPs: [list every ZIP found in the roster].' This set is now CLOSED — a property is covered ONLY if its exact ZIP code appears in this set. Do NOT group nearby ZIP codes. Do NOT infer coverage from property directories, geographic proximity, or any other non-roster source."

3. **Use ONLY the extracted table — the covered-key set is CLOSED** — all subsequent lookups MUST use the extracted table, NOT the raw source text, NOT zone labels, NOT geographic groupings from other databases. The step MUST say: "Use ONLY this extracted table for coverage decisions. The covered-key set is CLOSED: a key that is NOT in this set is NOT covered, regardless of where else it appears." CRITICAL: The key used for lookup MUST be the exact identifier from the roster (e.g., ZIP code), NOT a zone label or geographic grouping derived from a non-roster source. NEVER determine a property's key from the property directory — use only the key that appears in the roster.

4. **UNASSIGNED vs. backup-fallback — two distinct cases** — the step MUST distinguish:
   - **Key NOT in covered set** → UNASSIGNED (no primary, no backup exists in the roster for this key). Do NOT infer coverage from geographic proximity, zone labels, section headers in non-roster sources, or any other source. Do NOT assign to a nearby team member to fill the gap.
   - **Key IS in covered set, but primary is unavailable (day off) or over capacity** → assign to the roster-defined BACKUP for that key. UNASSIGNED is WRONG here. The roster names a backup precisely for this situation — use it. Only mark UNASSIGNED if the backup is also absent or the roster defines no backup for this key.
   The step MUST say: "For every work item whose key IS in the covered set: assign to the primary if available and within capacity; otherwise assign to the roster-defined backup for that key. Mark UNASSIGNED ONLY when the key is absent from the covered set entirely (no primary AND no backup in the roster)."

5. **Separate data sources by purpose** — each reference data source has ONE purpose. The step MUST declare which source is authoritative for each decision type. Example: "The staff directory is the ONLY source for coverage assignments. The property directory is the ONLY source for cleaning durations. NEVER use the property directory to determine coverage." CRITICAL: The coverage key (e.g., ZIP code) MUST come from the work item itself (e.g., the Hostfully checkout data), NOT from any Notion page. The roster maps keys → cleaners; the key is the identifier that appears in the work item. NEVER derive the coverage key from a pricing page, property directory, or any other non-roster, non-work-item source — those pages may group properties under incorrect zone labels.
