...
Specifically:
- **Capacity limits in reference data**: If the reference data (e.g., a staff directory) contains working hours or capacity limits per team member per day (e.g., "works Saturdays 11AM-3PM = 240 min"), the execution_steps MUST include a step that: (a) reads the capacity limits from the reference source, (b) calculates total work time per team member, (c) if total exceeds the capacity limit, assigns overflow to the backup person using property-address grouping (smallest-group-first, never split a single address across people).
...
2. **Extract a lookup table explicitly** — after reading, parse the content to build a structured lookup table. The step MUST say exactly what to extract and how. The content may be prose, bullets, or a table — reason over whatever structure is present. Example:
   > "Read the full content of Notion page [exact-id]. Parse it to build a ZIP-code → cleaner mapping by reading each section and extracting area codes and assigned names. Declare the full extracted table aloud."
...
1. **Extract a lookup table using the exact identifier the user provided**: After reading the reference data source (using the verbatim page ID/URL/name from the user's description), explicitly parse it to build a structured lookup table. The content may be prose, bullets, or a table — reason over whatever structure is present. Example: "Read the full content of Notion page [exact-id]. Parse it to build a ZIP → cleaner mapping by reading each section and extracting area codes and assigned names. Declare the full table aloud."
...
3. **Explicit UNASSIGNED**: State: "If a property's ZIP code is NOT found in the extracted table, mark it UNASSIGNED. Do NOT infer coverage from geographic proximity, zone groupings in non-roster directories, or any other non-authoritative source. A zone/area that appears in a property directory or trash directory does NOT establish that any team member covers it — only the roster source does."
...
- **Capacity limits in reference data**: If the reference data (e.g., a staff directory) contains working hours or capacity limits per team member per day (e.g., "works Saturdays 11AM-3PM = 240 min"), execution_steps MUST: (a) read the capacity limits from the reference source, (b) calculate total work time per team member, (c) if total exceeds the capacity limit, assign overflow to the backup person using property-address grouping (smallest-group-first, never split a single address across people).
