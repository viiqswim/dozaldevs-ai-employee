...
export interface AdminArchetypeConverseCreateRouteOptions {
  callLLM: typeof callLLM;
  prisma?: PrismaClient;
}

const ConverseCreateParamsSchema = z.object({
  tenantId: uuidField(),
});

const ConverseCreateBodySchema = z.object({
  transcript: z
    .array(
      z.object({
...
function buildEmptyBaseline(): GenerateArchetypeResponse {
  return {
    role_name: '',
...
export function adminArchetypeConverseCreateRoutes(
  opts: AdminArchetypeConverseCreateRouteOptions,
): Router {
  const router = Router();
  ...
  router.post(
    '/admin/tenants/:tenantId/archetypes/converse-create',
    authMiddleware,
    requireAuth,
    requireTenantRole(TenantRole.ADMIN),
    async (req, res) => {
      const paramResult = ConverseCreateParamsSchema.safeParse(req.params);
      ...
      const bodyResult = ConverseCreateBodySchema.safeParse(req.body);
      ...
      try {
        ...
        try {
          ...
          logger.warn({ err: persistErr }, 'Failed to persist converse-create call');
        }
        ...
        try {
          ...
          logger.warn({ err: persistErr }, 'Failed to persist converse-create failure');
        }
        ...
        logger.error({ err }, 'Archetype converse-create failed');
