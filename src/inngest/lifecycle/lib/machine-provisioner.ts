...
import type { EmployeeRuleRow } from '../../../workers/lib/postgrest-types.js';
import type { NotifyRef } from '../steps/triage-and-ready.js';
import { loadTenantSlack } from '../steps/notify-and-track.js';

const log = createLogger('machine-provisioner');

/** Maximum execution time for a Fly.io worker machine in seconds (30 minutes) */
const FLY_KILL_TIMEOUT_S = 1800;

export interface ProvisionMachineParams {
  taskId: string;
  archetypeId: string;
  tenantId: string;
  runId: string;
...
export async function provisionWorkerMachine(params: ProvisionMachineParams): Promise<string> {
  const {
    taskId,
    archetypeId,
    tenantId,
  ...
  const maxEmployeeRulesChars = parseInt(await getPlatformSetting('max_employee_rules_chars'), 10);
