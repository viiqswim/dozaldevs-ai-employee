...
describe('analyzeArchetype', () => {
  describe('toolIntensity', () => {
    it('returns none when instructions contain no tool keywords', () => {
    ...
    it('returns light when instructions contain a tool keyword', () => {
      const result = analyzeArchetype(
        makeArchetype({
          instructions: 'Fetch data from the slack api endpoint.',
        }),
      );
      expect(result.toolIntensity).toBe('light');
    });
  ...
  describe('outputQualityBar', () => {
    ...
    it('returns high when identity contains a quality keyword', () => {
      const result = analyzeArchetype(
        makeArchetype({
          identity: 'You are a professional assistant.',
          deliverable_type: 'task',
        }),
      );
      expect(result.outputQualityBar).toBe('high');
  ...
  describe('costSensitivity and domain', () => {
    ...
    it('detects hospitality domain when "guest" appears in identity or instructions', () => {
      const result = analyzeArchetype(
        makeArchetype({
          identity: 'Help guests with their stay.',
    ...
    it('detects engineering domain when "code" appears in combined text', () => {
      const result = analyzeArchetype(
        makeArchetype({
          identity: 'Review code submissions.',
    ...
    it('detects operations domain when "sifely" appears in combined text', () => {
      const result = analyzeArchetype(
        makeArchetype({
          identity: 'Rotate access for tenants.',
