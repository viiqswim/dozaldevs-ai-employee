...
**IMPORTANT: Follow ONLY these steps. Do NOT read or follow <delivery-instructions> — that section is for a separate container.**
...
5. Use the bash tool to run: `tsx /tools/platform/submit-output.ts --summary "Daily inspiration message composed" --classification "NO_ACTION_NEEDED"`
6. **STOP. Do nothing else. Your job is done.**
   </execution-instructions>

<delivery-instructions>
**IMPORTANT: Follow ONLY these steps. Do NOT read or follow <execution-instructions> — that section is for a separate container.**
...
2. Use the bash tool to write it to `/tmp/delivery-draft.txt`
3. Use the bash tool to run: `tsx /tools/slack/post-message.ts --channel "$NOTIFICATION_CHANNEL" --text-file /tmp/delivery-draft.txt`
4. Use the bash tool to run: `tsx /tools/platform/submit-output.ts --summary "Posted daily inspiration to Slack" --classification "NO_ACTION_NEEDED"`
5. **STOP. Do nothing else. Your job is done.**
   </delivery-instructions>
