...
## Task 26 — EmployeeList decomposition

### Files created

- `dashboard/src/components/StatusBadge.tsx` (35 lines) — extracted from EmployeeList.tsx lines 49-76; renders colored badge for archetype status values
- `dashboard/src/panels/employees/components/DeleteEmployeeDialog.tsx` — owns `deleteLoading` state + `handleDelete` logic; gets `tenantId` from `useTenant()` internally; prop: `archetype: Archetype | null` (null = closed)
- `dashboard/src/panels/employees/components/BulkDeleteDialog.tsx` — owns `bulkDeleteLoading` state + `handleBulkDelete` logic; gets `tenantId` from `useTenant()` internally
- `dashboard/src/panels/employees/components/EmployeeRowActions.tsx` — owns `loadingStates` state + all 6 action handlers (trigger, dryRun, restore, fireWebhook, setActive, setDraft); gets `tenantId`/`navigate` from hooks internally; uses `fireHostfullyWebhook` from `@/lib/gateway`
- `dashboard/src/panels/employees/components/EmployeeListStates.tsx` — exports `EmployeeListLoading`, `EmployeeListError`, `EmployeeListEmpty`; includes `SkeletonRow` (moved from EmployeeList.tsx)

### Key design decisions

1. **`deletingId: string | null` → `deletingArchetype: Archetype | null`** — cleaner design; passes full archetype object to DeleteEmployeeDialog instead of ID + name lookup. Dialog renders `archetype.role_name` directly.
2. **5 extractions needed (not 4) to hit <300 lines** — after extracting StatusBadge, dialogs, and row actions, file was still 348 lines. `EmployeeListStates.tsx` (loading/error/empty + SkeletonRow) pushed it to 278 lines.
3. **State fully moved to sub-components** — EmployeeList.tsx state reduced to only 3 items: `deletingArchetype`, `selected`, `bulkDeleteOpen`. All handler logic lives in the component that owns the state.
4. **LSP unavailable for dashboard** — typescript-language-server not in `.tool-versions`. `pnpm dashboard:build` is the only type-check authority.

### Verification

- `wc -l dashboard/src/panels/employees/EmployeeList.tsx` → 278 lines (< 300 ✓)
- `pnpm dashboard:build` → EXIT_CODE:0
