...
## Story Map Completion (2026-05-01)

- All 10 GM-22 acceptance criteria checkboxes marked `[x]` in `docs/planning/2026-04-21-2202-phase1-story-map.md`
- Use `awk '/#### GM-22:/,/^---/'` to scope grep to GM-22 section only (avoids capturing GM-15 items that follow)
- Committed: `docs(planning): mark GM-22 complete in story map`

## guest-message-poller Removal (2026-05-01)

- Deleted `src/inngest/triggers/guest-message-poller.ts` and `tests/inngest/triggers/guest-message-poller.test.ts`
- Removed 3 references from `src/gateway/inngest/serve.ts`: import, `const guestMessagePollerFn = ...`, and array entry
- `grep -r "guest-message-poller" src/` returns 0 results after removal
- `pnpm build` exits 0 — no orphaned type errors
- Committed: `refactor(inngest): remove guest-message-poller cron (replaced by webhook)`
