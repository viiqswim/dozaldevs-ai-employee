...
| `POST`   | `/admin/tenants/:tenantId/archetypes/:archetypeId/propose-edit`                   | Send a conversation transcript to the AI assistant; returns a clarifying question, a diff proposal, or a no-change signal (does not persist)                                                    |
| `POST`   | `/admin/tenants/:tenantId/archetypes/converse-create`                             | Send a conversation transcript to the AI assistant for CREATION (no existing archetype); returns a clarifying question, a proposal for a new employee, or a no-change signal (does not persist) |
...
| `POST`   | `/invitations/accept`                                                             | Accept an invitation by token (requires `Authorization: Bearer <jwt>`; membership-only — no user creation)                                                                                      |
...
| `generate-worker-constants.ts` | `pnpm generate-worker-constants`                                                                                   | Regenerate the World-B output-contract copy at `src/worker-tools/lib/output-contract-paths.generated.ts` from the World-A source. Run after changing `src/lib/output-contract-constants.ts`. CI fails if the committed copy is stale.                                                                                                            |
| `generate-tool-usage-skill.ts` | `pnpm generate-tool-usage-skill`                                                                                   | Regenerate the body of `src/workers/skills/tool-usage-reference/SKILL.md` from `ALL_TOOL_DESCRIPTORS`. Run after adding or changing a tool descriptor. Preserves hand-written content below the sentinel comment.                                                                                                                                |
| `generate-skills.ts`           | `pnpm generate-skills`                                                                                             | Regenerate all skills: tool-usage-reference (from `ALL_TOOL_DESCRIPTORS`), per-service custom-integration skills (hostfully, sifely, github, slack, knowledge-base, platform), and Composio skills (when `COMPOSIO_API_KEY` is set). Idempotent — safe to run repeatedly. CI has a freshness gate that fails if the committed folders are stale. |
...
| [Phase 1 Story Map](docs/planning/2026-04-21-2202-phase1-story-map.md)                      | Stories across multiple releases — active development roadmap                                                                                                                                                                                |
