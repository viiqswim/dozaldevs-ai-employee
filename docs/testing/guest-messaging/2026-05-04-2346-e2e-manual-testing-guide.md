...
```bash
grep -E "^(DATABASE_URL|SUPABASE_URL|SUPABASE_SECRET_KEY|INNGEST_EVENT_KEY|INNGEST_SIGNING_KEY|ENCRYPTION_KEY|SLACK_APP_TOKEN|SLACK_SIGNING_SECRET|OPENROUTER_API_KEY)=" .env
```
...
| `SERVICE_TOKEN`       | Machine-to-machine auth for admin API. Generate with `openssl rand -hex 32`. |
...
curl -s "http://localhost:7700/admin/tenants/00000000-0000-0000-0000-000000000003/secrets" \
  -H "Authorization: Bearer $SERVICE_TOKEN" | jq '.[] | select(.key == "hostfully_api_key")'
```
...
watch -n 5 "curl -s \
  ...
  -H 'Authorization: Bearer $SERVICE_TOKEN' | jq '{status}'"
```
...
curl -s "http://localhost:7700/admin/tenants/00000000-0000-0000-0000-000000000003/tasks/$TASK_ID" \
  -H "Authorization: Bearer $SERVICE_TOKEN" | jq '.deliverable.draftResponse'
```
...
curl -s "http://localhost:7700/admin/tenants/00000000-0000-0000-0000-000000000003/tasks/$TASK_ID" \
  -H "Authorization: Bearer $SERVICE_TOKEN" | jq '{id, status, metadata}'
```
...
while true; do
  STATUS=$(curl -s \
    "http://localhost:7700/admin/tenants/00000000-0000-0000-0000-000000000003/tasks/$TASK_ID" \
    -H "Authorization: Bearer $SERVICE_TOKEN" | jq -r '.status')
