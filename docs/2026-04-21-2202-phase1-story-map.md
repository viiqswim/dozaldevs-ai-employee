...
- [ ] Lifecycle always spawns a Fly.io machine for delivery after approval (no inline `slackClient.postMessage()`)
- [ ] Delivery machine reads approved content from the `deliverables` table (already written by the work machine)
- [ ] Each archetype has a `delivery_instructions` field (nullable text) — natural language telling the delivery machine what to do
- [ ] Summarizer delivery instructions: "Read the approved summary from the deliverable. Post it to the publish channel using `tsx /tools/slack/post-message.ts`."
- [ ] Guest messaging delivery instructions: "Read the approved response from the deliverable. Send it to the guest using `tsx /tools/hostfully/send-message.ts`."
- [ ] `DELIVERY_MODE` and `DELIVERY_MACHINE_ENABLED` env vars removed from codebase, `.env.example`, and all documentation
- [ ] All conditional delivery paths in `src/inngest/employee-lifecycle.ts` replaced with single machine-based delivery
- [ ] Delivery machine uses the same completion contract as work machine (`/tmp/summary.txt` or `/tmp/approval-message.json`, PATCH task status)
- [ ] Lifecycle polls delivery machine completion and handles failure (retry up to 2 times)
...
- [ ] `resolveAgentsMd()` concatenates all three levels with section headers (e.g., `# Platform Policy`, `# Tenant Conventions`, `# Employee Instructions`)
- [ ] If tenant or archetype level is null, that section is omitted (no empty headers)
- [ ] Platform level is always included (the static AGENTS.md from PLAT-02)
- [ ] Existing archetypes continue working (their `agents_md` becomes the third section, not the only content)
- [ ] Test: archetype with custom `agents_md` still includes platform security policy (self-repair, DB access rules)
...
- [ ] `tenants.config` JSON schema requires `notification_channel` (Slack channel ID)
- [ ] Prisma migration adds nullable `notification_channel` column to `archetypes` table
- [ ] Resolution logic: `archetype.notification_channel ?? tenant.config.notification_channel` — implemented in a shared utility
- [ ] `loadTenantEnv()` injects the resolved channel as `NOTIFICATION_CHANNEL` into machine env
- [ ] Existing VLRE and DozalDevs tenants seeded with appropriate `notification_channel` values
- [ ] Validation: tenant creation fails if `notification_channel` is missing from config
...
- [ ] No archetype `instructions` text contains hardcoded Slack channel IDs (e.g., `C092BJ04HUG`)
- [ ] Archetype instructions reference env vars (e.g., `$NOTIFICATION_CHANNEL`, `$SOURCE_CHANNELS`) instead
- [ ] `loadTenantEnv()` injects all necessary channel env vars from tenant config + archetype config
- [ ] Existing summarizer and guest messaging archetypes updated to use env var references
- [ ] `tenants.config.summary.channel_ids` → `tenants.config.source_channels` (or similar, rename to be generic)
- [ ] Stale env vars (`DAILY_SUMMARY_CHANNELS`, `SUMMARY_TARGET_CHANNEL`, `SUMMARY_PUBLISH_CHANNEL`) removed or aliased
- [ ] Changing a channel requires only a DB update — no code deploy or seed re-run
- [ ] `pnpm prisma db seed` runs without error
- [ ] **Manual test**: Change a channel ID in the DB, trigger a summarizer run, confirm it posts to the new channel
