...
- [ ] New archetype seeded in `prisma/seed.ts` with slug `unresponded-message-monitor`
- [ ] Inngest cron trigger fires every 30 minutes
- [ ] Employee reads the tenant's notification channel history using `tsx /tools/slack/read-channels.ts`
- [ ] Detection: identifies Block Kit messages containing guest messaging approval buttons (Approve/Reject/Edit & Send) that have **not** been replaced by a "Sent", "Rejected", or "Processing" confirmation block
- [ ] For each unacted card older than the configurable threshold (default: 30 minutes), posts a reminder thread reply tagging the responsible PM
- [ ] Reminder content: guest name, property, elapsed time since original card was posted, direct link to the pending approval card
...
- [ ] Urgent messages (maintenance emergencies, lock access issues) override quiet hours
- [ ] Reminders sent at most once per approval card per run (no repeated tagging)
- [ ] No-op run if no unacted cards are found (completes as "no work needed", not a failure)
