...
- [ ] New Express route: `POST /webhooks/hostfully` receives Hostfully `NEW_INBOX_MESSAGE` event payloads
- [ ] Returns HTTP 200 immediately — pipeline runs async (Hostfully expects fast ACK)
- [ ] Ignores any `event_type` other than `NEW_INBOX_MESSAGE` (returns `{ ok: true, ignored: true }`, still 200)
- [ ] Duplicate prevention: `message_uid` checked before processing — duplicate payloads are acknowledged but not re-processed (in-memory mark on receipt, persistent store on success, unmark on pipeline failure)
- [ ] On valid, non-duplicate payload: fires Inngest event triggering the guest messaging employee for that specific `message_uid` and `thread_uid`
- [ ] Registration script: `src/worker-tools/hostfully/register-webhook.ts` (replicates `vlre-employee/scripts/register-webhook.ts`) — lists existing webhooks, skips if already registered for same event+URL, registers `NEW_INBOX_MESSAGE` → gateway URL
- [ ] VLRE Hostfully account has `NEW_INBOX_MESSAGE` webhook registered pointing to the platform gateway
- [ ] `trigger/guest-message-poller` Inngest cron (from GM-09) removed from codebase
- [ ] `pnpm build` exits 0, `pnpm test -- --run` passes
- [ ] **End-to-end test on VLRE**: Send a test message via Hostfully, verify the approval card appears in Slack within 10 seconds
