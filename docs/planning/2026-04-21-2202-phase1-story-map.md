...
- [x] New shell tool: `src/worker-tools/locks/diagnose-access.ts --property-id "<id>"` (self-contained, inlines all Sifely auth + Hostfully door code fetch)
...
| **Porting notes** | **Net-new platform work.** The standalone MVP only reads from Sifely (list passcodes, access records). Mutation endpoints (add, change, delete passcode; list locks) exist in the Sifely TTLock API but were intentionally excluded from GM-15 to keep diagnosis read-only. This story extends `sifely-client.ts` in-place — no new files, no new shell tools, no new DB tables. |

**Acceptance Criteria:**

- [x] `sifely-client.ts` gains four new `--action` values, each using the corresponding Sifely TTLock API mutation endpoint:
  - `list-locks` — lists all locks registered on the Sifely account; output: `{ locks: [{ lockId, lockName, lockAlias, ... }] }`
  - `create-passcode --lock-id <id> --name <name> --code <code> [--start-date <ms>] [--end-date <ms>] [--type permanent|timed]` — creates a passcode on the lock; output: `{ keyboardPwdId: number }`
  - `update-passcode --lock-id <id> --passcode-id <id> [--name <name>] [--start-date <ms>] [--end-date <ms>]` — updates an existing passcode's name or validity window; output: `{ ok: true }`
  - `delete-passcode --lock-id <id> --passcode-id <id>` — permanently removes a passcode from the lock; output: `{ ok: true }`
