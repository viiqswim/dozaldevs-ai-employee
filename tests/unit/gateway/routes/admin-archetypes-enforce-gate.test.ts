...
describe('PATCH /admin/tenants/:tenantId/archetypes/:archetypeId — enforce_tool_registry gate', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('400 — flipping enforce_tool_registry true with empty tool_registry is rejected', async () => {
    const existing = makeArchetype({ tool_registry: { tools: [] } });
    const findFirst = vi.fn().mockResolvedValue(existing);
    const update = vi.fn();

    const app = makeApp({ archetype: { findFirst, update } });

    const res = await request(app)
      .patch(`/admin/tenants/${TENANT}/archetypes/${ARCHETYPE_ID}`)
      .send({ enforce_tool_registry: true });

    expect(res.status).toBe(400);
    expect(res.body.error).toBe('ENFORCE_REGISTRY_INVALID');
    expect(update).not.toHaveBeenCalled();
  });

  it('400 — flipping enforce_tool_registry true with unresolvable tool paths is rejected', async () => {
    const existing = makeArchetype({
      tool_registry: { tools: ['/tools/nonexistent/bogus.ts'] },
    });
    const findFirst = vi.fn().mockResolvedValue(existing);
    const update = vi.fn();
  ...
  it('200 — flipping enforce_tool_registry true with all-resolvable paths succeeds', async () => {
    const existing = makeArchetype({
      tool_registry: { tools: ['/tools/slack/post-message.ts'] },
    });
    ...
    const archetypeEditHistory = { create: vi.fn().mockResolvedValue({}) };

    const app = makeApp({ archetype: { findFirst, update }, archetypeEditHistory });
  ...
  it('200 — flipping enforce_tool_registry false (disable) is always allowed', async () => {
    const existing = makeArchetype({ enforce_tool_registry: true, tool_registry: { tools: [] } });
    const updated = makeArchetype({ enforce_tool_registry: false, tool_registry: { tools: [] } });
    const findFirst = vi.fn().mockResolvedValue(existing);
    const update = vi.fn().mockResolvedValue(updated);
    const archetypeEditHistory = { create: vi.fn().mockResolvedValue({}) };
    ...
    const res = await request(app)
      .patch(`/admin/tenants/${TENANT}/archetypes/${ARCHETYPE_ID}`)
      .send({ enforce_tool_registry: false });
