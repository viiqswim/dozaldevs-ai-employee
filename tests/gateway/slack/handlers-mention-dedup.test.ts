...
describe('app_mention deduplication', () => {
  ...
  it('different ts values → inngest.send called exactly twice', async () => {
    ...
    await handler({ event: makeMentionEvent({ ts: '222.001', channel: 'C_DIFF_TS' }) });
    await handler({ event: makeMentionEvent({ ts: '222.002', channel: 'C_DIFF_TS' }) });
  ...
  it('same ts but different channel → inngest.send called exactly twice', async () => {
    ...
    await handler({ event: makeMentionEvent({ ts: '444.001', channel: 'C_CHAN_A' }) });
    await handler({ event: makeMentionEvent({ ts: '444.001', channel: 'C_CHAN_B' }) });
